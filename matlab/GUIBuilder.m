% GUIBuilder.m - A mixin class, with methods for creating interactive GUIs for objects
%
...
classdef GUIBuilder < handle  
    ...
    properties (Transient=true, Hidden=true)
        disableWaitbar = false; % Set to true to disable the waitbar
    end
    ...
    methods (Static=true)
        ...
        function pos = autoSizePanel(panelH)
            ...
            pos = panelH.Position;
            cpos = CellFun.cellmatfun(@getpixelposition, panelH.Children); %children positions
            if ~isempty(cpos)
        ...
        function pos = autoSizeTabGroup(tabG)
            ...
            g_sz = [0,0];
            for n = 1:numel(tabs)
                cpos=CellFun.cellmatfun(@getpixelposition, tabs(n).Children); %children positions
                if ~isempty(cpos)
        ...
        function bbox=align(H, varargin)
            ...
            % positions for each of a set of uicontrols, but does not actually move anything around.  This is a
            % simple wrapper around align that allows a version of align that actually moves around the
            % objects.
            % [IN]
            %   H - cellarray of handles
            %   [Align args] - All other arguments are passed directly to align.  In general you need to
            %   use the same arguments here as you would calling matlab's 'align' function.
            % [OUT]
            %   bbox - [x y w h]: a bounding box in Matlab position format for the outer edge of the
            %                     aligned uicontrol elements.
            if isstruct(H)
                H = struct2cell(H);
            end
            if isscalar(H)
            ...
            for n=1:numel(H)
                H{n}.Position = new_pos(n,:); %Actually move the uicontrol elements.  Matlab 'align' doesn't do this for you.
            end
       ...
       function menuH = makeFigureMenu(figH, title, labels, CBs)
            ...
            %  labels - cell array of strings - the labels on the menu, blanks [] make separators
            ...
            CBs = CellFun.cellmap(@GUIBuilder.safeCallback,CBs); % Make callbacks safe
        ...
        function hJTable = getJTableHandle(tableH)
            ...
            %
            %  This relies on the well established Matlab file exchange program findjobj
        ...
        function clearJTableCallbacks(tableH)
            % Should be called on GUI close to ensure we clean up and can free the object.  This is
        ...
        function positionImageAxes(axesH, imageSize, area, labelMargin)
            ...
            if ~isempty(imageSize) 
                ...
                tot_inset = inset(1:2)+inset(3:4); %Total inset amount in [x y]
                ...
                axesH.OuterPosition = area; %No image, so give up and just set the outer position
            end
        end
        ...
        function positionAxes(axesH, area, labelMargin)
            ...
            %  area - [x y w h]: Matlab position format - the maximum area the axes can cover
            %  labelMargin - [left bottom right top] - the amount to pad the outside of the axes to
        ...
        function clearPropertyGridMenus(tableHs)
            ...
            %tableHs is a cell array of property grids to clear menu for
            tableHs = CellFun.makecell(tableHs);
